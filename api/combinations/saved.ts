...
import { getSupabaseServerClient, requireUserIdFromAuthHeader } from '../_supabaseServer';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      const { error } = await supabase.from('saved_combinations').insert(insertData);
      if (error) throw error;
    ...
    if (req.method === 'DELETE') {
      ...
      if (clear) {
        const { error } = await supabase.from('saved_combinations').delete().eq('user_id', userId);
        if (error) throw error;
        return res.status(200).json({ success: true });
      }

      if (!id) return res.status(400).json({ error: 'Missing id (or clear=true)' });
      const { error } = await supabase.from('saved_combinations').delete().eq('id', id).eq('user_id', userId);
      if (error) throw error;
      return res.status(200).json({ success: true });
    }
