...
// Analyze saved combinations for near misses against winning extractions
export const analyzeSavedCombinationsForNearMisses = (
  savedCombinations: GeneratedCombination[],
  extractions: ExtractedNumbers[],
  gameType: GameType,
  wheel?: LottoWheel
): NearMissAnalysis => {
  // Filter saved combinations for the specific game type and wheel
  ...
  if (relevantCombinations.length === 0 || extractions.length === 0) {
    return {
      nearMisses: [],
      totalAnalyzed: 0,
      criteriaUsed: 'Nessuna combinazione o estrazione disponibile per l\'analisi'
    };
  }

  const nearMisses: NearMissResult[] = [];

  // Analyze each saved combination against all extractions
  relevantCombinations.forEach(combo => {
    extractions.forEach(extraction => {
      // Get the relevant winning numbers based on game type and wheel
      let winningNumbers: number[];
      ...
      // Analyze matches for this combination vs this extraction
      const matches: NearMissMatch[] = [];
      let exactMatches = 0;
      let offByOneMatches = 0;

      combo.numbers.forEach(savedNumber => {
        // Check for exact match
        if (winningNumbers.includes(savedNumber)) {
          matches.push({
            number: savedNumber,
            type: 'exact'
          });
          ...
          // Check for off-by-one matches
          const offByOneNumbers = [savedNumber - 1, savedNumber + 1];
          const matchingWinningNumber = winningNumbers.find(winNum => 
            offByOneNumbers.includes(winNum)
          );
          
          if (matchingWinningNumber) {
            matches.push({
              number: savedNumber,
              type: 'off-by-one',
              winningNumber: matchingWinningNumber
            });
            offByOneMatches++;
          }
        }
      ...
      // Calculate total score (exact matches worth more than off-by-one)
      const totalScore = (exactMatches * 2) + offByOneMatches;

      // Only include if it meets our "near miss" criteria
      // At least 3 total matches (exact + off-by-one) OR at least 2 exact matches
      if (matches.length >= 3 || exactMatches >= 2) {
        nearMisses.push({
  ...
  // Sort by total score (best matches first), then by date (most recent first)
  nearMisses.sort((a, b) => {
  ...
  return {
    nearMisses: nearMisses.slice(0, 20), // Limit to top 20 results
    totalAnalyzed: relevantCombinations.length,
    criteriaUsed: 'Almeno 3 numeri totali (esatti + vicini) O almeno 2 numeri esatti'
  };
};

// Get insights from near miss analysis
export const getNearMissInsights = (analysis: NearMissAnalysis): string[] => {
  const insights: string[] = [];

  if (analysis.nearMisses.length === 0) {
    insights.push("❌ Nessuna delle tue combinazioni salvate è stata vicina ai numeri vincenti secondo i criteri di analisi.");
    insights.push(`📊 Analizzate ${analysis.totalAnalyzed} combinazioni salvate.`);
    return insights;
  }

  const bestMatch = analysis.nearMisses[0];
  insights.push(`🎯 La tua migliore "quasi-vincita" ha totalizzato ${bestMatch.exactMatches} numeri esatti e ${bestMatch.offByOneMatches} numeri "vicini di 1" il ${new Date(bestMatch.extractionDate).toLocaleDateString('it-IT')}.`);

  const totalExactMatches = analysis.nearMisses.reduce((sum, match) => sum + match.exactMatches, 0);
  const totalOffByOneMatches = analysis.nearMisses.reduce((sum, match) => sum + match.offByOneMatches, 0);

  insights.push(`📈 In totale: ${totalExactMatches} numeri esatti e ${totalOffByOneMatches} numeri "vicini di 1" nelle tue ${analysis.nearMisses.length} migliori quasi-vincite.`);

  // Analyze patterns in near misses
  const frequentNearMissNumbers: Record<number, number> = {};
  analysis.nearMisses.forEach(nearMiss => {
    nearMiss.matches.forEach(match => {
      frequentNearMissNumbers[match.number] = (frequentNearMissNumbers[match.number] || 0) + 1;
    });
  });

  const topNearMissNumbers = Object.entries(frequentNearMissNumbers)
    .sort(([,a], [,b]) => b - a)
    .slice(0, 5)
    .map(([num]) => parseInt(num));

  if (topNearMissNumbers.length > 0) {
    insights.push(`🔥 I tuoi numeri che sono stati più spesso "vicini" alle vincite: ${topNearMissNumbers.join(', ')}.`);
  }

  insights.push(`📋 Criteri utilizzati: ${analysis.criteriaUsed}`);
