...
import { TrendingDown, Trash2, Plus, BarChart3, AlertTriangle, Target, TrendingUp, Search, Zap } from 'lucide-react';
import { useGame } from '../../context/GameContext';
import { getGameByType } from '../../utils/generators';
import { analyzeUnsuccessfulCombinations, getAnalysisInsights, analyzeSavedCombinationsForNearMisses, getNearMissInsights } from '../../utils/analysisUtils';
...
const UnsuccessfulCombinations: React.FC = () => {
  ...
  const [showNearMissAnalysis, setShowNearMissAnalysis] = useState(false);
  ...
  // Get near miss analysis for saved combinations
  const nearMissAnalysis = analyzeSavedCombinationsForNearMisses(
    savedCombinations,
    extractionsData[selectedGame],
    selectedGame,
    selectedGame === 'lotto' ? selectedWheel : undefined
  ...
  const nearMissInsights = getNearMissInsights(nearMissAnalysis);
  ...
  return (
    <div className="card mb-8">
        ...
        <div className="flex space-x-2">
          ...
          {gameCombinations.length > 0 && (
            <>
              ...
              <button
                onClick={() => setShowNearMissAnalysis(!showNearMissAnalysis)}
                className="btn btn-outline flex items-center text-sm"
              >
                <Search className="h-4 w-4 mr-1" />
                {showNearMissAnalysis ? 'Nascondi Quasi-Vincite' : 'Quasi-Vincite'}
              </button>
            </>
          )}
        </div>
      ...
      {showNearMissAnalysis && (
        <div className="mb-6 p-4 bg-bg-primary border border-gray-200 dark:border-gray-700 rounded-lg">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold flex items-center">
              <Search className="h-5 w-5 mr-2 text-primary" />
              Analisi Quasi-Vincite delle Combinazioni Salvate
            </h3>
          ...
          <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md">
            <div className="flex items-start">
              <Zap className="h-5 w-5 text-blue-600 dark:text-blue-400 flex-shrink-0 mr-2 mt-0.5" />
              <div className="text-sm">
                <p className="font-medium text-blue-800 dark:text-blue-200 mb-1">
                  Analisi Quasi-Vincite
                </p>
                <p className="text-blue-700 dark:text-blue-300">
                  Questa analisi confronta le tue combinazioni salvate con i numeri vincenti storici, 
                  identificando i casi in cui i tuoi numeri erano molto vicini (esatti o "off by 1") 
                  ai numeri vincenti. Ad esempio: il tuo 60 vs il vincente 59 o 61.
                </p>
              </div>
            </div>
          </div>
          ...
          {/* Near Miss Insights */}
          <div className="mb-6">
            <h4 className="text-md font-medium mb-3">🎯 Risultati Analisi:</h4>
            <div className="space-y-2">
              {nearMissInsights.map((insight, index) => (
                <div key={index} className="p-2 bg-bg-secondary rounded text-sm">
                  {insight}
                </div>
              ))}
          ...
          {/* Near Miss Results */}
          {nearMissAnalysis.nearMisses.length > 0 && (
            <div>
              <h4 className="text-md font-medium mb-3">🔍 Dettagli Quasi-Vincite (Top {nearMissAnalysis.nearMisses.length}):</h4>
              <div className="space-y-4 max-h-96 overflow-y-auto">
                {nearMissAnalysis.nearMisses.map((nearMiss, index) => (
                  <div key={index} className="p-4 bg-bg-secondary border border-gray-200 dark:border-gray-700 rounded-lg">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm text-text-secondary">
                          {new Date(nearMiss.extractionDate).toLocaleDateString('it-IT')}
                        </span>
                        {nearMiss.wheel && (
                      ...
                      <div className="flex items-center space-x-3 text-sm">
                        <span className="text-success font-medium">
                          {nearMiss.exactMatches} esatti
                        </span>
                        <span className="text-warning font-medium">
                          {nearMiss.offByOneMatches} vicini
                        </span>
                        <span className="text-primary font-medium">
                          Score: {nearMiss.totalScore}
                        </span>
                      </div>
                    </div>
                    ...
                    {/* Your Numbers */}
                    <div className="mb-3">
                      <p className="text-xs text-text-secondary mb-2">I tuoi numeri:</p>
                      <div className="flex flex-wrap gap-2">
                        {nearMiss.savedCombination.numbers.map((num, i) => {
                          const match = nearMiss.matches.find(m => m.number === num);
                          const type = match ? (match.type === 'exact' ? 'selected' : 'hot') : 'default';
                          return (
                            <div key={i} className="relative">
                              <NumberBubble number={num} type={type} size="sm" />
                              {match && match.type === 'off-by-one' && (
                                <div className="absolute -top-1 -right-1 bg-warning text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">
                                  {match.winningNumber}
                                </div>
                              )}
                            </div>
                    ...
                    {/* Winning Numbers */}
                    <div>
                      <p className="text-xs text-text-secondary mb-2">Numeri vincenti:</p>
                      <div className="flex flex-wrap gap-2">
                        {nearMiss.winningNumbers.map((num, i) => (
                          <NumberBubble key={i} number={num} type="cold" size="sm" />
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="mt-2 text-xs text-text-secondary">
                      Strategia: {nearMiss.savedCombination.isAdvancedAI ? 'AI Avanzata' : 
                                nearMiss.savedCombination.isAI ? 'AI Locale' : 
                                nearMiss.savedCombination.strategy === 'standard' ? 'Standard' : 'Alta Variabilità'}
                      {' • '}
                      Salvata: {new Date(nearMiss.savedCombination.date).toLocaleDateString('it-IT')}
                    </div>
                  </div>
                ))}
              </div>
                ...
                >
                  ...
                  <td className="py-3 px-4 border-b border-gray-200 dark:border-gray-700">
                    <button
                      ...
                      onClick={() => {
                        deleteUnsuccessfulCombination(combo.id).catch((error) => {
          ...
          <button
            ...
            onClick={() => {
              if (confirm('Sei sicuro di voler eliminare tutte le combinazioni non vincenti? Questo resetterà l\'apprendimento dell\'AI.')) {
                clearUnsuccessfulCombinations().catch((error) => {
                  showToast.error('Errore durante l\'eliminazione: ' + error.message);
                });
              }
            }}
